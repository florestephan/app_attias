...
const Form3 = (props) => {
  ...
  const continued = e => {
    ...
    firebase.user(Date.now()).set({
      "Nom de famille": values.nom_de_famille,
      "Prénom": values.prenom,
      "DOB": values.date_de_naissance,
      "Sexe": values.sexe,
      "Fumeur": values.tabac,
      "Dormez-vous seul(e)": values.dormir_seul,
      "Profession": values.job,
      "Ronflement": values.ronflement,
      "Maux de tête le matin ?": values.maux_de_tete,
      "Pauses respiratoires la nuit ?": values.pause_respiratoire,
      "Nomnres d'urine par nuit ?": values.uriner_nuit,
      "Problèmes de mémoire la journée ?": values.probleme_memoire,
      "Etes vous irritable la journée ?": values.irritable,
      "Sommeil est réparateur ?": values.sommeil_reparateur,
      "Somnolence au volant ?": values.somnolance_volant,
      "Accident endormissement au volant ?": values.accident_endormissement_volant,
      pichot,
      epworth,
      hadA,
      hadD
